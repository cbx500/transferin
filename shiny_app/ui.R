...
ui <- dashboardPage(
  ...
  # Reduced sidebar width for more room on the main dashboard page
  dashboardSidebar(
    width = 200,
    sidebarMenu(
      ...
      tabItem(tabName = "instructions",
              fluidRow(
                box(
                  ...
                  br(),
                  p(strong("Additional Instructions for the Personal Data file:")),
                  p("You only need to update the following fields:"),
                  tags$ul(
                    tags$li("Staff_Name"),
                    tags$li("DOB_Staff"),
                    tags$li("Gender_Staff"),
                    tags$li("DOB_Spouse"),
                    tags$li("Gender_Spouse"),
                    tags$li("Retirement_Date_Staff")
                  ),
                  p("The Personal Data Excel file contains two sheets:"),
                  tags$ul(
                    tags$li(strong("PersonalData:"), " All fields in this sheet must be filled in, even if some duplicate previous information. The field 'revalued_earningsHR' should be taken from the transfer in paper provided by HR and represents the monthly revalued earnings at retirement age discounted at report date."),
                    tags$li(strong("SalaryHistory:"), " Fill this sheet if you wish to recalculate your projected revalued earnings. The 'Value' column should include your past monthly basic salaries for each year since the scheme started, and the 'Worked' column (values between 0 and 1) indicates the proportion of time worked (1 represents full time). For example, for 2009 enter 0.5833333, reflecting that the scheme began on 1st June 2009.")
                  )
                )
              )
      ),
