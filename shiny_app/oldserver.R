...
server <- function(input, output, session) {
  ...
  # File paths for the input files (always relative for deployment)
  ...
  ## Personal Information (from Extracted Personal Data)
  ...
  ## Revalued Earnings Information (from results file)
  output$annual_revalued_earnings_hr <- renderText({ 
    ...
    paste("Annual Revalued Earnings HR:", format_number(val))
  })
  output$annual_revalued_earnings_hr_capitalised <- renderText({ 
    ...
    paste("Annual Revalued Earnings HR Capitalised:", format_number(val))
  })
  output$revalued_earnings_retirement_age65 <- renderText({ 
    ...
    paste("Revalued Earnings at Retirement Age 65:", format_number(val))
  })
  output$revalued_earnings_discounted <- renderText({ 
    ...
    paste("Revalued Earnings (Discounted to Report Date):", format_number(val))
  })
  
  ## Transfer-In Credit Results (from results file)
  ...
  output$discount_rate_nby <- renderText({
    ...
    paste("Discount Rate NBY:", format_number(val))
  })
  output$preretirement_discountrate <- renderText({
    ...
    paste("Pre-retirement Discount Rate:", format_number(val))
  })
